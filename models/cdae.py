...
class CDAE(object):
    ...
    def get_graph(self):
        ...
        with tf.variable_scope('encode'):
            ...
            # Normal Encoder Part
            encode_weights = tf.Variable(tf.random_normal([self.input_dim, self.embed_dim],
                                                            stddev=1 / (self.embed_dim ** 0.5), dtype=tf.float32),
                                         name="Weights")
            encode_bias = tf.Variable(tf.constant(0., shape=[self.embed_dim]), name="Bias")

            # User Representation
            user_embeddings = tf.Variable(tf.random_normal([self.user_dim, self.embed_dim],
                                                            stddev=1 / (self.embed_dim ** 0.5), dtype=tf.float32),
                                          name="UserEmbeddings")

            users = tf.nn.embedding_lookup(user_embeddings, self.user_id, name="users")
        ...
        with tf.variable_scope('decode'):
            self.decode_weights = tf.Variable(tf.random_normal([self.embed_dim, self.output_dim],
                                                            stddev=1 / (self.embed_dim ** 0.5), dtype=tf.float32),
                                         name="Weights")
...
def cdae(matrix_train, embeded_matrix=np.empty((0)), iteration=100, lam=80, rank=200, corrpution=0.8, seed=1, **unused):
    ...
    model.train_model(matrix_input, 0.8, iteration)
